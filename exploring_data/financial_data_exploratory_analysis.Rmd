...
fig <- fin_data_tech %>% filter(year == 2017, quarter == qt, ticker == tick) %>% ggplot(aes(x=date)) + geom_line(aes(y = close)) + geom_line(aes(y = `SMA 14`, color='SMA 14')) + geom_line(aes(y = `SMA 21`, colour='SMA 21')) + geom_line(aes(y = `SMA 42`, color='SMA 42')) + scale_color_discrete(name='SMA', labels = waiver()) + theme_grey() + ggtitle(tick)
plot(fig)
  }
}
```
...
```{r}
fig1 <- fin_data_tech_trend %>% filter(ticker == 'DIA.MC') %>% ggplot(aes(x = date)) + geom_line(aes(y=close))
fig2 <-  fin_data_tech_trend %>% filter(ticker == 'DIA.MC') %>% ggplot(aes(x = date)) + geom_line(aes(y = `ADX 42`, colour = 'ADX 42')) + geom_line(aes(y = `MINUS_DI 42`), color='coral1') + geom_line(aes(y = `PLUS_DI 42`), color='darkblue')
plot_grid(fig2, fig1, align = 'v', nrow = 2, rel_heights = c(2/3, 1/3))
```
